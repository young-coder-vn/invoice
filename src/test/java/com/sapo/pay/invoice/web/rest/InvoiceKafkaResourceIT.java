...
import com.sapo.pay.invoice.config.KafkaProperties;
...
class InvoiceKafkaResourceIT {
    ...
    void setup() {
        ...
        InvoiceKafkaResource kafkaResource = new InvoiceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/invoice-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/invoice-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
